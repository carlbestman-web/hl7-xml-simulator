...
var mServiceCode = if(!isGuestOrder) vars.serviceCode else ((obx filter (upper($.'OBX.3'.'CE.1') ~= upper(p('obx-identifier.reg-accommodation-rate'))))[0].'OBX.5')
...
---
{
	udto#UDTO: {
				...
				udto#slmc_cs: {
					...
					cs#slb: {
						slb#orders: {
							...
							slb#order: {
								slb#ci_no: orc.'ORC.2'.'EI.1',
								slb#order_grp_no: orc.'ORC.2'.'EI.1',
								slb#order_pkg_no: "",
								slb#package_rate_no: "",
								slb#requesting_unit: pv1.'PV1.3'.'PL.1',
								...
								slb#validated_by: if(!isGuestOrder) orc.'ORC.11'.'XCN.1' else orc.'ORC.10'.'XCN.1',
								slb#order_dtl: {
									slb#order_dtl_no: orc.'ORC.2'.'EI.1',
									...
									slb#quantity: if(!isGuestOrder) orc.'ORC.7'.'TQ.1'.'CQ.1' else ((obx filter (upper($.'OBX.3'.'CE.1') ~= upper(p('obx-identifier.reg-rate-multiplier'))))[0].'OBX.5'),
									slb#dose: rxo.'RXO.2',
									...
									slb#generic_code: "",
									slb#route_code: vars.prerequisiteInfo.routeCode default ((obx filter (upper($.'OBX.3'.'CE.1') ~= upper(p('obx-identifier.route-code'))))[0].'OBX.5'),
									slb#batch_tag: "",
									slb#package_tag: "",
									slb#late_order_flag: "",
									slb#pnf_flag: "",
		...
		udto#EventKey: Mule::p('event-keys.emr-to-slb-new-order')
	}
}